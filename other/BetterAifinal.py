...
#GHUB.mouse_open()
from DdxoftMouse import DdxoftMouse
...
def restart_program():
    ...
    print("[AI] -> restarting, please wait...")
...
class Protect():
    ...
    def start(self):
        ...
        def warn(message):
            ...
            print(f"[AI] -> Blacklisted process detected. You have been warned")
...
def download_requirements():
    ...
    for module in modules:
        ...
        print(f"[AI] -> installing requirements.. ({int((i/total_modules)*100)}%)")
    ...
    print("[AI] -> successfuly installed packages")

    print("[AI] -> restarting program...")
...
try:
    ...
    if os.path.exists(r'C:\\ProgramData\\LizAI\\icons\\logo.png'):
...
try:
    ...
    Gui_color = QColor(*config.get('Gui_color', (121, 0, 245, 1)))
...
def download_assets():
    ...
    paths = ['C:\\ProgramData\\LizAI\\icons', 'C:\\ProgramData\\LizAI\\font', 'C:\\ProgramData\\LizAI\\models']
    ...
    if not os.path.exists(r'C:\\ProgramData\\LizAI\\icons\\logo.png'):

        print("[AI] -> downloading icons...")

        zip_path = r'C:\\ProgramData\\LizAI\\icons\\icons.zip'

        with open(zip_path, 'wb') as f:

            f.write(requests.get('https://www.dropbox.com/scl/fi/ecsw790l3wzmcvk4bbdks/icons.zip?rlkey=z2dux1uft0z45reokxedfuddo&st=4gjfkto9&dl=1').content)



        with zipfile.ZipFile(zip_path, 'r') as zip_ref:

            zip_ref.extractall(r'C:\\ProgramData\\LizAI')
        ...
        try:
            ...
            print(f"[AI] -> error removing zip file: {e}")



    if not os.path.exists(r'C:\\ProgramData\\LizAI\\font\\Font.ttf'):

        print("[AI] -> downloading font...")

        zip_path = r'C:\\ProgramData\\LizAI\\font\\font.zip'
    ...
    if not os.path.exists(r'C:\\ProgramData\\LizAI\\models\model.pt'):

        print("[AI] -> downloading models...")

        zip_path = r'C:\\ProgramData\\LizAI\\models\\model.zip'

        with open(zip_path, 'wb') as f:

            f.write(requests.get('https://www.dropbox.com/scl/fi/i89wlruilht4mb2g4go5u/models.zip?rlkey=4qiqkbovqthvxo93mxtr53xw7&st=yt175kxc&dl=1').content)
...
# Initial model load
load_model("C:/ProgramData/LizAI/models/" + AI_Model)
...
def find_arduino_port():
    """Find Arduino-like device, including spoofed names like 'USB Serial Device'."""
    known_vids = ['2341', '2A03']  # Arduino SA, Arduino LLC
    ports = list_ports.comports()

    for port in ports:
        desc = port.description.lower() if port.description else ''
        if 'arduino' in desc or 'usb serial device' in desc:
            print(f"[AI] -> Found possible Arduino: {port.device} ({port.description})")
            return port.device
        if port.vid and format(port.vid, '04x') in known_vids:
            print(f"[AI] -> Found Arduino by VID: {port.device}")
            return port.device

    print("[AI] -> No Arduino device found.")
    return None

# --- Utility ---
#def is_logitech_running():
    #for proc in psutil.process_iter(['name']):
        #try:
            #if 'lghub_agent.exe' in proc.info['name'].lower():
                #return True
...
class ClickableLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            webbrowser.open("https://discord.gg/zenscript")
...
class api:
    ...
    def license(self, key, hwid=None):
        ...
        if json["success"]:
            ...
            key_file = open(rf"{os.getcwd()}\license.AI", 'w')
            ...
            input("[AI] -> invalid, expired or used license key\n")
...
class License():
    ...
    def auto_login():
        ...
        try:
            ...
            key_file = open(rf"{os.getcwd()}\license.AI", "r")
...
class GUI(QWidget):
    ...
    def toggle_overlay(self, state):
        print("Overlay Enabled" if state == Qt.Checked else "Overlay Disabled")
    ...
    def safe_mouse_move(self, x, y):
        try:
            if self.Mouse_Movement == "Default" and hasattr(self, "ddxoft") and self.ddxoft:
                self.ddxoft.move(x, y)
            elif self.Mouse_Movement == "DDXoft":
                self.ddxoft.move(0, x, y, 0)
            else:
                self.ddxoft.move(0, x, y, 0)
        except Exception as e:
        ...
        def on_click(x, y, button, pressed):
            if self.Norecoil_checkbox.isChecked():
                if button == mouse.Button.left:
                    self.is_left_held = pressed
                    if pressed:
                        self.safe_mouse_move(0, Norecoil_senstivity)
                        print("Left Click - Moving Down")
                elif button == mouse.Button.right:
                    self.is_right_held = pressed
                    if pressed:
                        self.safe_mouse_move(0, -Norecoil_senstivity)
    ...
    def adjust_mouse_position(self):
        # Move using HID input instead of system cursor
        self.safe_mouse_move(0, self.Norecoil_offset)
    ...
    def init_ui(self):
        ...
        self.setWindowIcon(QIcon("C:/ProgramData/LizAI/icons/logo.png"))
        ...
        font_id = QFontDatabase.addApplicationFont("C:/ProgramData/LizAI/font/Font.ttf")
        ...
        self.mouse_movement_ddxoft = QRadioButton("Default")
        self.mouse_movement_ddxoft.setStyleSheet(self.get_radio_button_style())
        self.mouse_movement_group.addButton(self.mouse_movement_ddxoft)

        self.mouse_movement_arduino = QRadioButton("Arduino")
        self.mouse_movement_arduino.setStyleSheet(self.get_radio_button_style())

        #self.mouse_movement_ghub = QRadioButton("GHUB")
        #self.mouse_movement_ghub.setStyleSheet(self.get_radio_button_style())
        ...
        self.mouse_movement_group.addButton(self.mouse_movement_makcu)
        #self.mouse_movement_group.addButton(self.mouse_movement_ghub)
        ...
        def update_mouse_movement(button):
            ...
            self.set_widgets_visible([self.Norecoil_label, self.Norecoil_checkbox,self.Norecoil_slider], self.Mouse_Movement == "arduino")
        ...
        if self.Mouse_Movement == "Default":
            ...
            self.mouse_movement_ddxoft.setChecked(True)

        elif self.Mouse_Movement == "Arduino":
        ...
        #elif self.Mouse_Movement == "GHUB":
            #self.mouse_movement_ghub.setChecked(True)
        #else:
            #self.mouse_movement_default.setChecked(True)
        ...
        self.humanization_bspline = QRadioButton("BSpline")

        self.humanization_bspline.setStyleSheet(self.get_radio_button_style())
        ...
        self.humanization_group.addButton(self.humanization_bspline)
        ...
        elif self.Humanization == "BSpline":

            self.humanization_bspline.setChecked(True)
        ...
        btn_aimbot.setIcon(QIcon(f"C:/ProgramData/LizAI/icons/aimbot.png"))
        ...
        btn_visual.setIcon(QIcon(f"C:/ProgramData/LizAI/icons/visuals.png"))
        ...
        btn_profile.setIcon(QIcon(f"C:/ProgramData/LizAI/icons/profile.png"))
        ...
        aimbot_horizontal_separator.setStyleSheet("background-color: #7900f5; height: 1px;")
        ...
        humanization_line_2.addWidget(self.humanization_bspline)
        ...
        visuals_horizontal_separator.setStyleSheet("background-color: #7900f5; height: 1px;")
        ...
        profile_horizontal_separator.setStyleSheet("background-color: #7900f5; height: 1px;")
        ...
        gmarket_logo_pixmap = QPixmap("C:/ProgramData/LizAI/icons/logo.png").scaled(47, 47, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        discord_logo_pixmap = QPixmap("C:/ProgramData/LizAI/icons/discord.png").scaled(47, 47, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        separator_vertical.setStyleSheet("background-color: #7900f5; width: 1px; ")
        ...
        def set_button_style(selected_button):
            ...
            btn_aimbot.setIcon(QIcon(f"C:/ProgramData/LizAI/icons/aimbot-highlighted.png") if selected_button == "Aimbot" else QIcon("C:/ProgramData/LizAI/icons/aimbot.png"))



            btn_visual.setStyleSheet(self.menu_tab_selected_style() if selected_button == "Visual" else self.menu_tab_selected_style())

            btn_visual.setIcon(QIcon("C:/ProgramData/LizAI/icons/visuals-highlighted.png") if selected_button == "Visual" else QIcon("C:/ProgramData/LizAI/icons/visuals.png"))



            btn_profile.setStyleSheet(self.menu_tab_selected_style() if selected_button == "Profile" else self.menu_tab_selected_style())

            btn_profile.setIcon(QIcon("C:/ProgramData/LizAI/icons/profile-highlighted.png") if selected_button == "Profile" else QIcon("C:/ProgramData/LizAI/icons/profile.png"))
    ...
    def update_models(self):
        ...
        models_path = 'C:\\ProgramData\\LizAI\\models'
    ...
    def get_toggle_button_style(self):
        ...
        return f"""
            ...
            QCheckBox::indicator:checked {{
                ...
                image: url(C:/ProgramData/LizAI/icons/o-right.png);
            ...
            QCheckBox::indicator:unchecked {{
                ...
                image: url(C:/ProgramData/LizAI/icons/o-left.png);
    ...
    def get_combo_box_style(self):
        ...
        return f"""
            ...
            QComboBox::down-arrow {{
                ...
                image: url(C:/ProgramData/LizAI/icons/d.png);
...
class Visuals_Overlay(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(3, 15, f"omencheats.com ") 
...
#class GHUBMouse:
    #def __init__(self):
        #if not is_logitech_running():
            #raise RuntimeError("Logitech G HUB (lghub_agent.exe) is not running. Please start it.")
        #try:
            #GHUB.mouse_open()
            #self.initialized = True
            #print("[GHUB] -> Connected to Logitech G HUB successfully.")
        #except Exception as e:
            #print(f"[GHUB] -> Failed to initialize GHUB: {e}")
            #self.initialized = False

    #def move(self, x, y):
        #if not is_logitech_running():
            #print("[GHUB] -> G HUB is not running. Movement blocked.")
            #return
        #try:
            #GHUB.mouse_move(0, int(x), int(y), 0)
        #except Exception as e:
            #print(f"[GHUB] -> move error: {e}")
            #GHUB.mouse_open()

    #def click(self, button_name="left", press=True):
        #if not is_logitech_running():
            #print("[GHUB] -> G HUB is not running. Click blocked.")
            #return
        #button_map = {"left": 1, "right": 2, "middle": 4}
        #value = button_map.get(button_name.lower(), 0)
        #if not press:
            #value = 0
        #try:
            #GHUB.mouse_move(value, 0, 0, 0)
        #except Exception as e:
            #print(f"[GHUB] -> click error: {e}")
            #GHUB.mouse_open()
...
class ArduinoMouse:
    def __init__(self, port, baudrate=9600):
        try:
            ...
            print(f"[AI] -> Connected to Arduino mouse on {port}")
        except serial.SerialException as e:
            print(f"[AI] -> Failed to connect to Arduino: {e}")
            raise
    ...
    def move(self, x, y):
        try:
            ...
            print(f"[AI] -> Failed to send to Arduino: {e}")
            raise
    ...
    def press(self):
        try:
            ...
            print(f"[AI] -> Failed to send press: {e}")
            raise
    ...
    def release(self):
        try:
            ...
            print(f"[AI] -> Failed to send release: {e}")
            raise
...
class DummyUser:
    ...
    subscription = {"name": "Ultimate", "expiry": "1893456000"}
    subscriptions = [{"name": "Ultimate", "expiry": "1893456000"}]

import types
api = types.SimpleNamespace()
api.user_data = DummyUser()
...
class AI:
    ...
    def __init__(self, gui):
        ...
        self.hid_mouse = None
        self.makcu = None
        self.ddxoft_mouse = None
        self.ghub_mouse = None
    ...
    def b_spline_interpolation(self, p0, p1, p2, p3, t):
        t2 = t * t
        t3 = t2 * t
        # Cubic B-spline basis functions (uniform)
        B0 = ((1 - t)**3) / 6
        B1 = (3*t3 - 6*t2 + 4) / 6
    ...
    #def mouse_click(button_name: str, press: bool):
        #button_map = {"left": 1, "right": 2, "middle": 4}
        #value = button_map.get(button_name.lower(), 0)
        #if not press:
            #value = 0
    ...
    def move_mouse(self, x, y, humanization=None):
        ...
        elif humanization == "CatmullRom":
            # You'd need four control points; this is just a placeholder
            move_x = self.catmull_rom_interpolation(0, 0.25 * move_x, 0.75 * move_x, move_x, t)
            move_y = self.catmull_rom_interpolation(0, 0.25 * move_y, 0.75 * move_y, move_y, t)
        elif humanization == "BSpline":
            t = distance / 100
            # Provide four control points similarly (you can adjust control point weights)
            move_x = self.b_spline_interpolation(0, 0.25 * move_x, 0.75 * move_x, move_x, t)
            move_y = self.b_spline_interpolation(0, 0.25 * move_y, 0.75 * move_y, move_y, t)
        ...
        with self.lock:
            if self.gui.Mouse_Movement == "Arduino":
                if self.hid_mouse is None:
                    try:
                        # Auto-detect Arduino port instead of hardcoding COM5
                        port = find_arduino_port()
                        if port is None:
                            print("[AI] -> Arduino mouse not found! Aiming disabled.")
                            return
                        self.hid_mouse = ArduinoMouse(port=port)
                    except Exception as e:
                        print(f"[AI] -> Failed to initialize Arduino mouse: {e}")
                        return
                try:
                    self.hid_mouse.move(int(move_x), int(move_y))
                except Exception as e:
                    print(f"[AI] -> Error moving Arduino mouse: {e}")
                    self.hid_mouse = None

            #elif self.gui.Mouse_Movement == "GHUB":
                #if self.ghub_mouse is None:
                    #try:
                        #self.ghub_mouse = GHUBMouse()
                    #except Exception as e:
                        #print(f"[AI] -> Failed to initialize GHUB mouse: {e}")
                        #return
                #try:
                    #self.ghub_mouse.move(int(move_x), int(move_y))
                #except Exception as e:
                    #print(f"[AI] -> Error moving GHUB mouse: {e}")
                    #self.ghub_mouse = None


            elif self.gui.Mouse_Movement == "Default":
                if self.ddxoft_mouse is None:
                    try:
                       self.ddxoft_mouse = DdxoftMouse()
                    except Exception as e:
                        print(f"[AI] -> Failed to initialize DDXoft mouse: {e}")
                        return
                if self.ddxoft_mouse.ready:
                    try:
                        self.ddxoft_mouse.move(int(move_x), int(move_y))
                    except Exception as e:
                        print(f"[AI] -> Error moving DDXoft mouse: {e}")
                        self.ddxoft_mouse = None
                ...
                if self.makcu is None:
                    ...
                    if not self.makcu.connect():
                        print("[AI] -> Makcu device not found! Aiming disabled.")
                        self.makcu = None
                        return
                if self.makcu:
                    try:
                       self.makcu.move(int(move_x), int(move_y))
                        ...
                        print(f"[AI]-> Error moving MAKCU: {e}")
                        try:
                           self.makcu.close()
                        except:
                            pass
